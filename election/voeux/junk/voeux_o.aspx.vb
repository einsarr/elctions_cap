...
Partial Class candidature_candidature
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Try
            'Si la session de l'enseignant est tjrs active
            If Session("id_ens") <> "" Then
                possibilite = nosFonc.CandidaturePossible(Session("id_ens"))
                If possibilite = False Then
					'S'il ne peut candidater on le redirige vers voeux fermer
	...
	Protected Sub VerifData()
        ...
        If DropSpc.SelectedValue.ToString <> "" Then
            Try
                Dim sqlException As New SqlCommand( _
                       "SELECT code_type_mouvement from fup_exception_demande " & _
                       "where code_type_mouvement = (select code_type_mouvement from fup_mouvement where code_mouvement = '" & Session("c_mvmt") & "')" & _
                       " and code_fonction_agent = '" & Session("c_fct") & "'" & _
                       " and code_specialite_agent = '" & Session("c_spc") & "'" & _
    ...
    Protected Sub BtAddVoeux_Click(sender As Object, e As EventArgs) Handles BtAddVoeux.Click
        ...
        Try
            Dim sqlInsertVoeux As New SqlCommand("INSERT INTO FUP_DEMANDE_POSTE_VACANT ( " & _
                                                 "CODE_ETABLISSEMENT, " & _
                                                 "CODE_FONCTION, " & _
                                                 "CODE_SPECIALITE, " & _
                                                 "CODE_MOUVEMENT, " & _
                                                 "IDENTIFIANT_ENSEIGNANT," & _
                                                 "CODE_ORDRE_PREFERENCE, " & _
                                                 "CREE_PAR, " & _
                                                 "CODE_ANNEE )" & _
                                                 "SELECT  '" & _
                                                 Me.DropEtab.SelectedValue.ToString & "','" & _
                                                 Me.DropFct.SelectedValue.ToString & "','" & _
                                                 Me.DropSpc.SelectedValue.ToString & "','" & _
                                                 Me.DropMvmt.SelectedValue.ToString & "','" & _
                                                 Session("id_ens") & "','" & _
                                                 Me.DropOrdre.SelectedValue.ToString & "','" & _
                                                 Session("t_matricule") & "'," & _
    ...
    Protected Sub AddNote()
        Try
            ...
            Dim sqlInsertNote As New SqlCommand("INSERT INTO FUP_NOTE_ENSEIGNANT_EN_LIGNE ( " & _
                                                 "IDENTIFIANT_ENSEIGNANT, " & _
                                                 "CODE_ANNEE, " & _
                                                 "CODE_CRITERE, " & _
                                                 "CREE_PAR )" & _
                                                 ...
                                                 "ANNEE.CODE_ANNEE, " & _
                                                 "FUP_CRITERE_FICHE_NOTATION.CODE_CRITERE, " & _
                                                  "'" & Session("t_matricule").ToString & "'" & _
                    "FROM         ENSEIGNANT INNER JOIN " & _
                      "FUP_DEMANDE_POSTE_VACANT ON ENSEIGNANT.IDENTIFIANT_ENSEIGNANT = FUP_DEMANDE_POSTE_VACANT.IDENTIFIANT_ENSEIGNANT INNER JOIN " & _
                      "FUP_MOUVEMENT ON FUP_DEMANDE_POSTE_VACANT.CODE_MOUVEMENT = FUP_MOUVEMENT.CODE_MOUVEMENT INNER JOIN " & _
                      "FUP_TYPE_MOUVEMENT ON FUP_MOUVEMENT.CODE_TYPE_MOUVEMENT = FUP_TYPE_MOUVEMENT.CODE_TYPE_MOUVEMENT INNER JOIN " & _
                      "FUP_FICHE_MUTATION ON FUP_TYPE_MOUVEMENT.CODE_FICHE_MUTATION = FUP_FICHE_MUTATION.CODE_FICHE_MUTATION INNER JOIN " & _
                      "FUP_CRITERE_FICHE_NOTATION ON FUP_FICHE_MUTATION.CODE_FICHE_MUTATION = FUP_CRITERE_FICHE_NOTATION.CODE_FICHE_MUTATION INNER JOIN " & _
                      "FUP_CRITERE ON FUP_CRITERE_FICHE_NOTATION.CODE_CRITERE = FUP_CRITERE.CODE_CRITERE INNER JOIN " & _
                      "FUP_CRITERE_SEXE ON FUP_CRITERE.CODE_CRITERE = FUP_CRITERE_SEXE.CODE_CRITERE AND " & _
                      "ENSEIGNANT.SEXE_ENSEIGNANT = FUP_CRITERE_SEXE.SEXE INNER JOIN " & _
                      "ANNEE ON FUP_DEMANDE_POSTE_VACANT.CODE_ANNEE = ANNEE.CODE_ANNEE AND FUP_MOUVEMENT.CODE_ANNEE = ANNEE.CODE_ANNEE " & _
                      "WHERE    (ANNEE.ANNEE_ENCOURS = 1)  " & _
...
"From FUP_MOUVEMENT INNER Join " &
	...
	Protected Sub GridViewDossierNum_DataBound(sender As Object, e As EventArgs) Handles GridViewDossierNum.DataBound
        ...
        If Me.GridViewDossierNum.PageCount > 0 Then
            ...
            Me.LabelDepot.Text = "Tu dois déposer ta fiche de vœux avec les justificatifs au niveau de ton inspection avant le <b class=""text-warning"">" & dateCloture.ToString("D") & " à 17h</b>. La commission locale procédera à la validation de ta candidature au mouvement national."
		End If
