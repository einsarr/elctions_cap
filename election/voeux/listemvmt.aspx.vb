...
#End Region
    ...
    'Get actes enseignant
    Function getNbreActeEnseignant(ByVal _IDENTIFIANT_ENSEIGNANT As Integer) As Integer
        Dim template As New DataTable("template")
        ...
        Try
            ...
            Dim query As String = "SELECT COUNT(ID) AS NbreActe " &
                                  "FROM ACTE_NUMERISE " &
                                  " WHERE (IDENTIFIANT_ENSEIGNANT=" & _IDENTIFIANT_ENSEIGNANT & ") "
            '& " AND (ISNULL(VALIDE, 0) = 1) "

            Dim com As New SqlCommand(query, dbConnection)
        ...
        Dim row As DataRow = template.Rows(0)

        NbreActe = row("NbreActe").ToString()
